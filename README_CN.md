...
- **工具与 MCP** — 60+ 工具,进程内与 MCP 同名同义,另有交互式图谱视图 `render_graph_html`,以及会分页的批量列举工具 `memory_list_all`、`graph_list_all`——大脑大到一页装不下时,返回 `next_cursor` 续读下一页。
