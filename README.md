...
- **Tools & MCP** — 60+ tools with the same names in-process and over MCP, plus `render_graph_html`, an interactive graph view, and paged bulk listings (`memory_list_all`, `graph_list_all`) — `graph_list_all` defaults to the most-connected core; `order: "id"` walks the whole graph instead, returning `next_cursor` page by page like `memory_list_all` does.
