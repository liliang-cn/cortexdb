...
//
// On both backends, because a property survives a write here by being read
// back out of a JSON column and written again, and the two databases do not
// store or decode that column the same way.
//
...
func TestUpsertEntitiesKeepsPropertiesTheCallerDidNotMention(t *testing.T) {
	for _, backend := range decisionBackends(t) {
